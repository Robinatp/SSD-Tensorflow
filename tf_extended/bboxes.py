...
def bboxes_nms_all_classes(classes,scores, bboxes, nms_threshold=0.5, keep_top_k=200, scope=None):
    """Apply non-maximum selection to bounding boxes. In comparison to TF
    implementation, use classes information for matching.
    Should only be used on single-entries. Use batch version otherwise.
...
def bboxes_nms_batch_all_classes(classes,scores, bboxes, nms_threshold=0.5, keep_top_k=200,
                     scope=None):
    """Apply non-maximum selection to bounding boxes. In comparison to TF
    implementation, use classes information for matching.
    Use only on batched-inputs. Use zero-padding in order to batch output
    ...
    """
    # Dictionaries as inputs.
    if isinstance(classes, dict) or isinstance(scores, dict) or isinstance(bboxes, dict):
        with tf.name_scope(scope, 'bboxes_nms_batch_dict'):
            d_classes = {}
            d_scores = {}
            d_bboxes = {}
            for c in scores.keys():
                cl,s, b = bboxes_nms_batch_all_classes(classes[c],scores[c], bboxes[c],
                                        nms_threshold=nms_threshold,
                                        keep_top_k=keep_top_k)
                d_classes[c] = cl                        
                d_scores[c] = s
                d_bboxes[c] = b
            return d_classes, d_scores, d_bboxes

    # Tensors inputs.
    with tf.name_scope(scope, 'bboxes_nms_batch'):
        r = tf.map_fn(lambda x: bboxes_nms_all_classes(x[0],x[1],x[2],
                                           nms_threshold, keep_top_k),
        ...
        return tf.squeeze(classes,0),  tf.squeeze(scores,0),  tf.squeeze(bboxes,0)
